...
/-- The matter-gravity projector kills the one-dimensional complement. -/
theorem Pmg_omega : Pmg omega = 0 := by
  unfold Pmg
  simp [inner, omega_norm]

/-- The matter-gravity projector fixes vectors orthogonal to omega. -/
theorem Pmg_eq_self_of_omega_perp {Y : SceneC1} (h : omega_perp Y) :
    Pmg Y = Y := by
  unfold Pmg
  have h' : inner Y omega = 0 := h
  rw [h']
  simp

/-- The matter-gravity projector is idempotent. -/
theorem Pmg_idempotent (Y : SceneC1) : Pmg (Pmg Y) = Pmg Y :=
  Pmg_eq_self_of_omega_perp (Pmg_omega_perp Y)

/-- Sum functional on the zone-11 row-coefficient space. -/
...
/-- Centered coefficients on the 11-zone: the row coefficients have zero total sum. -/
def CenteredRowCoeff : Submodule ℚ (V11T → ℚ) :=
  LinearMap.ker rowCoeffSum

/-- The centered row-coefficient space has dimension 10 = 11 - 1. -/
theorem CenteredRowCoeff_finrank :
  ...
  have h_dom : Module.finrank ℚ (V11T → ℚ) = 11 := by
    rw [Module.finrank_fintype_fun_eq_card]
    simp
  rw [h_dom] at h
  omega

/-- Embed centered 11-zone row coefficients into the literal edge carrier:
zero on the (9,11) and (9,13) blocks, constant in the V13 coordinate on (11,13). -/
...
theorem rowCorrectionEmbed_injective :
  ...
  intro a ha
  ext y
  have h := congrFun ha (Sum.inr (Sum.inr (y, (0 : V13T))))
  simpa [rowCorrectionEmbed] using h

/-- The actual rank-10 correction space used by U-I. -/
def RowCorrectionSpace : Submodule ℚ SceneC1 :=
...
/-- U differs from the identity only through the 10-dimensional centered row-correction space. -/
theorem U_minus_I_in_RowCorrectionSpace (X : SceneC1) :
    U X - X ∈ RowCorrectionSpace := by
  let a : V11T → ℚ := fun y =>
    -(2 / 13 : ℚ) *
      ((∑ z' : V13T, X (Sum.inr (Sum.inr (y, z')))) -
        (1 / 11 : ℚ) *
          ∑ y' : V11T, ∑ z' : V13T, X (Sum.inr (Sum.inr (y', z'))))
