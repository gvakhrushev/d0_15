...
def _search_forms(text: str) -> set[str]:
    """Normalized forms used for matching punctuation-heavy identifiers."""
    raw = _strip_diacritics(str(text)).lower()
...
def _query_terms(question: str) -> list[str]:
    ...
    for raw in question.split():
        if _has_chinese(raw):
            ...
            raw_tokens = re.findall(r"\w+", raw.lower())
            if len(raw_tokens) > 1:
                terms.append(" ".join(raw_tokens))
                terms.append("".join(raw_tokens))
...
def _score_nodes(G: nx.Graph, terms: list[str]) -> list[tuple[float, str]]:
    ...
    for term in terms:
        ...
        if len(tokens) > 1:
            norm_terms.append(" ".join(tokens))
            norm_terms.append("".join(tokens))
        norm_terms.extend(tokens)
    norm_terms = list(dict.fromkeys(norm_terms))
    idf = _compute_idf(G, norm_terms)
    for nid, data in G.nodes(data=True):
        norm_label = data.get("norm_label") or _strip_diacritics(data.get("label") or "").lower()
        bare_label = norm_label.rstrip("()")
        label_forms = _search_forms(norm_label) | _search_forms(bare_label) | _search_forms(nid)
        ...
        for t in norm_terms:
            ...
            elif any(form.startswith(t) for form in label_forms):
                score += _PREFIX_MATCH_BONUS * w
            elif any(t in form for form in label_forms):
...
def _find_node(G: nx.Graph, label: str) -> list[str]:
    ...
    """
    term_forms = _search_forms(label)
    if not term_forms:
        return []
    exact: list[str] = []
    prefix: list[str] = []
    ...
    for nid, d in G.nodes(data=True):
        ...
        bare_label = norm_label.rstrip("()")
        node_forms = _search_forms(norm_label) | _search_forms(bare_label) | _search_forms(nid)
        if term_forms & node_forms:
            exact.append(nid)
        elif any(node_form.startswith(term) for term in term_forms for node_form in node_forms):
            prefix.append(nid)
        elif any(term in node_form for term in term_forms for node_form in node_forms):
