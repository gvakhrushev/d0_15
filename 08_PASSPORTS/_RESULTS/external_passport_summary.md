...
| SPARC shape_only kernel | sparc_archive_phason_halo | 08_PASSPORTS/SPARC/sparc_manifest.json | dfbdbb678ce5ec98 | naive inner-memory archive-phason kernel | baryon-only residual shape + amplitude/inner/outer/LSB-HSB/gas-disk | dominant failure modes classified (amplitude, inner slope, LSB etc.); no new kernel until full diagnostics | FAIL_ARCHIVE_PHASON_HALO_SPARC_SHAPE_ONLY | failure diagnostics complete; recommendation: reject simple radial smoothing; next operator must include hull-boundary response |
...
| IceCube HESE phason decoherence | icecube_hese12 | 08_PASSPORTS/IceCube/icecube_manifest.json | 8b3ebbc0a8ad6ea0 | neutral bulk phason-wave kernel with finite decoherence window | no-decoherence flat survival null + shape metric | HESE event curve generated; simple shape metric vs null implemented; full exposure/flux baseline still required | SKIP_NEUTRINO_PHASON_DECOHERENCE_BASELINE_REQUIRED | no_decoherence_null: flat survival; d0_vs_null_shape_metric implemented; baseline_ready: false (exposure/flux needed) |
...
| BAO S_DE phason flip | desi_bao_sde_phason_flip | 08_PASSPORTS/DESI/desi_dr2_manifest.json | 52756a8144355414 | S_DE phason-flip archive transfer | constant-shape BAO baseline | BAO residual/covariance response | FAIL_DESI_BAO_SDE_REAL_DATA + PASS_DESI_BAO_SDE_FAILURE_DIAGNOSTICS | official pinned data, no refit, dominant: HIGH_Z_FAILURE, AMPLITUDE_TOO_LOW, MID_Z_FAILURE |
| DESI/BAO S_DE real-data hard run | DESI_DR2_BAO | 08_PASSPORTS/DESI/desi_dr2_manifest.json | (see manifest) | Frozen S_DE (λ_c=1.420943, λ_r=1.579057, no refit) | ΛCDM published + flat residual | delta_chi2, rmse on sample bins | FAIL_DESI_BAO_SDE_REAL_DATA | Data from bao_data repo (official source). No refit. delta_chi2=463. Physical result on the obtained data. |
