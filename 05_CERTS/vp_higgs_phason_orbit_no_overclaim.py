#!/usr/bin/env python3
"""vp_higgs_phason_orbit_no_overclaim - block higgs-phason-orbit overclaim while named blockers remain.
...
caught. Phrases licensed only if HIGGS_PHASON_ORBIT_BLOCKERS.csv shows zero global blockers (it does not).
"""
import csv
import pathlib
import re
...
FORBIDDEN = [
    "D0 derives the Higgs VEV", "D0 derives 246 GeV", "D0 derives Higgs mass",
    "quartic Higgs potential proved", "electroweak symmetry breaking fully closed",
    "Higgs condensate derived", "246 GeV derived",
]
...
def main() -> int:
    print("=== vp_higgs_phason_orbit_no_overclaim  no higgs-phason-orbit grand-overclaim while named blockers remain ===")
    ...
    if blockers:
        assert not hits, f"affirmative higgs-phason-orbit over-claim(s) while {len(blockers)} blockers remain: {hits}"
    ...
    assert blockers, "expected named higgs-phason-orbit blockers (frontier must stay non-empty)"
    print(f"PASS_FRONTIER_NONEMPTY  {len(blockers)} blockers named in HIGGS_PHASON_ORBIT_BLOCKERS.csv.")
    planted = "result: D0 derives the Higgs VEV and electroweak symmetry breaking fully closed."
    ...
    honest = "D0 does not derive the Higgs VEV, and 246 GeV is not derived."
    ...
    print("PASS_HIGGS_PHASON_ORBIT_NO_OVERCLAIM")
    return 0
