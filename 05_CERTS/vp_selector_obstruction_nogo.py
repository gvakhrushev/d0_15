...
def count_fixed_bijections_under(group, n):
    # Number of bijections b in Bij(V_n,L_n) fixed by EVERY g in `group` — the
    # equivariant sections w.r.t. that group.  SHARED code path for the main
    # theorem (group = full S_n) and the guard-grade negative control below
    # (a group action with free orbits of size 1, where sections DO exist).
    Bij = bijections(n)         # bijections V_n -> L_n (label alphabet size n)
    fixed = 0
    for b in Bij:
        if all(act(g, b) == b for g in group):
            fixed += 1
    return fixed

def count_fixed_bijections(n):
    # main-theorem instance: the FULL S_n action (Aut-equivariant sections)
    return count_fixed_bijections_under(bijections(n), n)
...
# ---------------------------------------------------------------------------
# GUARD-GRADE NEGATIVE CONTROL (can fail the CONCLUSION, not the technique):
# run the SAME section-count on a group action whose orbits are free of size 1 —
# the TRIVIAL group {id} acting on Bij(V_n,L_n).  There, equivariant sections
# DO exist (every bijection is one: n! sections).  If the shared counter
# reported 0 here, the scene's "0 sections" would be an artifact of the
# counting machinery, not of the S_n action — and this cert would EXIT 1.
# ---------------------------------------------------------------------------
print("\n" + "="*70)
print("NEGATIVE CONTROL: trivial-group action (free orbits of size 1) MUST have sections")
print("="*70)
for n in [2, 3]:
    trivial_group = [tuple(range(n))]                       # {identity} only
    fx_triv = count_fixed_bijections_under(trivial_group, n)  # COMPUTED, same code path
    check(f"CTRL_TRIVIAL_GROUP_SECTIONS_EXIST_n{n}", fx_triv == factorial(n),
          f"trivial group on Bij(V_{n},L_{n}): {fx_triv} fixed bijections found (>0: sections exist)")
    assert fx_triv > 0, "negative control found NO sections where sections must exist — counter broken"
    if fx_triv > 0:
        print(f"NEGATIVE_CONTROL_CAUGHT: n={n}: trivial-group action admits {fx_triv} computed "
              f"sections — the scene's 0-section result is a property of the S_n action, not the counter")

# technique gate: act() must be a genuine (left) group action — composition law,
# verified on explicit elements (a broken act() would silently poison every count).
_g1, _g2, _b0 = (1, 0, 2), (0, 2, 1), (2, 0, 1)
_comp = tuple(_g1[_g2[i]] for i in range(3))                # g1∘g2
assert act(_g1, act(_g2, _b0)) == act(_comp, _b0), "act() violates the group-action composition law"
assert act(tuple(range(3)), _b0) == _b0, "act() violates identity law"
