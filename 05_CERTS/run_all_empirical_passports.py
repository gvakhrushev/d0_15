...
scripts=['vp_v1133_bao_sde_reproducible_passport.py','vp_cosmology_entropy_flow_likelihood.py']
results=[]
for s in scripts:
    p=here/s
    if not p.exists():
...
open('run_all_empirical_passports_results.json','w').write(json.dumps({'pass':pass_all,'results':results},indent=2))
print('PASS_RUN_ALL_EMPIRICAL_PASSPORTS' if pass_all else 'FAIL_RUN_ALL_EMPIRICAL_PASSPORTS')
