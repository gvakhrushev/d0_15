#!/usr/bin/env python3
"""
vp_selector_ssb_nogo.py  — registry cert for D0-CANONICAL-WITHIN-ZONE-SELECTOR-M1-NOGO-001
                            (SSB arm). Minted copy of _TASKS_CENTER_ATTACK/selector_ssb_check.py
                            with the ONLY change (per SELECTOR_MECHANISM_REPORT.md EoR-A): the
                            two supplementary general-commutant checks now use the EXACT Reynolds
                            projector onto the S9xS11xS13 commutant instead of a 400-sample
                            approximation. The 400-sample version left Ugen a NON-exact commutant
                            element (order ~1e19 group), so `in_commutant` read False and
                            ||P Ugen Q|| ~ 0.5 were SAMPLING NOISE, not a real P<->Q cross-block —
                            the load-bearing F2 class-function no-go never depended on them. The
                            exact projector makes the witness genuinely non-symmetric AND exactly
                            in the commutant, so PUQ=0 holds exactly (as the R1 dim-12 commutant
                            theorem requires) and the cert exits 0. No load-bearing check altered.
...
# a GENERIC element of the S9xS11xS13 commutant (not merely a polynomial in the
# symmetric L). A generic commutant element still must give PUQ=0 (no
# trivial<->std cross-block, R1 cert dim 12), else regime (i) vacuity would be an
# artifact of the symmetric-poly subspace.
#
# EoR-A FIX (vs the memo's 400-sample reynolds_average): we build the EXACT
# Reynolds projector onto the commutant. Averaging M over all of S9xS11xS13 sends
# entry (i,j) to the mean over the ORBIT of (i,j). For distinct zone sizes the
# orbits are exactly: {within-zone-a diagonal}, {within-zone-a off-diagonal},
# {zone-a -> zone-b block} for each ordered pair a!=b. So the exact projection is
# a per-orbit constant fill — computed here in closed form (no sampling). The
# result is EXACTLY in the commutant (commutes with every within-zone permutation
# to machine precision) and, by seeding the source block-means asymmetrically,
# genuinely NON-symmetric (Ugen != Ugen^T).
def exact_reynolds_projection(M):
    """Exact average of M over G=S9xS11xS13 = per-(zone-pair, diag/offdiag) mean fill."""
    out = np.zeros_like(M, dtype=float)
    for za in range(len(SIZES)):
        ia = np.where(zone_of == za)[0]
        for zb in range(len(SIZES)):
            jb = np.where(zone_of == zb)[0]
            block = M[np.ix_(ia, jb)]
            if za != zb:
                out[np.ix_(ia, jb)] = block.mean()          # single off-zone orbit
            else:
                na = len(ia)
                diag_mean = np.trace(block) / na
                off_sum = block.sum() - np.trace(block)
                off_mean = off_sum / (na * (na - 1)) if na > 1 else 0.0
                out[np.ix_(ia, jb)] = off_mean
                np.fill_diagonal(out[np.ix_(ia, jb)], diag_mean)
    return out
# Seed with an asymmetric source so the projected commutant element is non-symmetric:
# make the zone-a->zone-b block mean differ from the zone-b->zone-a block mean.
Mseed = rng.standard_normal((N, N))
for za in range(len(SIZES)):
    for zb in range(len(SIZES)):
        if za < zb:                       # bias the two directed blocks differently
            ia = np.where(zone_of == za)[0]; jb = np.where(zone_of == zb)[0]
            Mseed[np.ix_(ia, jb)] += 3.0  # a->b block lifted; b->a block untouched
Ugen = exact_reynolds_projection(Mseed)   # EXACT generic commutant element
# verify it is genuinely non-symmetric (not a poly in the symmetric L)
is_nonsym = np.linalg.norm(Ugen - Ugen.T) > 1e-3
# verify it commutes with EVERY within-zone permutation, exactly (in the commutant)
in_commutant = True
for _ in range(30):
    perm = within_zone_perm(rng); Pm = perm_matrix(perm)
    if not np.allclose(Pm @ Ugen, Ugen @ Pm, atol=1e-9):
        in_commutant = False; break
...
check("F2 (i) GENERAL commutant witness is non-symmetric & in commutant",
      ...
      f"||Ugen-Ugen^T||={np.linalg.norm(Ugen-Ugen.T):.3f}, commutes(exact)={in_commutant}")
...
# NC5 (guard-grade, can fail the CONCLUSION): a NON-TRANSITIVE action admitting a
# UNIQUE NON-SYMMETRIC minimizer.  The scene verdict rests on the trichotomy
# "invariant functional of a TRANSITIVE within-zone action => extrema symmetric
# or degenerate-orbit, never a unique broken point (c)".  Control: take H =
# Stab(vertex 0) < S9 on zone 9 — NON-transitive (orbits {0} and {1..8}, both
# computed below) — and the H-invariant potential p = 1 - indicator(vertex 0).
# The SAME extremum analysis must then find outcome (c): a UNIQUE minimizer
# (vertex 0, computed argmin) that is NOT S9-symmetric (its full-S9 orbit has
# size 9, computed).  If the machinery failed to see this unique broken
# minimizer, the scene conclusion "no outcome (c)" would be machinery blindness
# and this cert EXITS 1.
p_ctrl = np.ones(9); p_ctrl[0] = 0.0                 # H-invariant potential on zone 9
def stab0_perm(rng):
    return np.concatenate(([0], 1 + rng.permutation(8)))   # fix 0, permute {1..8}
h_inv = all(np.array_equal(p_ctrl[stab0_perm(rng)], p_ctrl) for _ in range(60))
# non-transitivity of H, computed: orbit of vertex 0 = {0}; orbit of vertex 1 never hits 0
orb_v0 = {int(stab0_perm(rng)[0]) for _ in range(60)}
orb_v1 = {int(np.where(stab0_perm(rng) == 1)[0][0]) for _ in range(200)}
h_nontransitive = (orb_v0 == {0}) and (0 not in orb_v1) and len(orb_v1) > 1
# unique minimizer, computed argmin
mins = np.where(p_ctrl == p_ctrl.min())[0]
unique_min = (len(mins) == 1) and (int(mins[0]) == 0)
# the minimizer is NOT S9-symmetric: the full-S9 orbit of the selected vertex is all 9
s9_orbit_of_min = set()
for t in range(9):                                    # transposition (0,t) images, computed
    perm = np.arange(9); perm[0], perm[t] = perm[t], perm[0]
    s9_orbit_of_min.add(int(np.where(perm == int(mins[0]))[0][0]))
not_s9_symmetric = (len(s9_orbit_of_min) == 9)
# and p itself is NOT S9-invariant (a transposition moves it), computed
perm01 = np.arange(9); perm01[0], perm01[1] = 1, 0
p_not_s9_inv = not np.array_equal(p_ctrl[perm01], p_ctrl)
nc5 = h_inv and h_nontransitive and unique_min and not_s9_symmetric and p_not_s9_inv
check("NC5: NON-transitive action admits a UNIQUE NON-SYMMETRIC minimizer (outcome (c) reachable)",
      nc5,
      f"H=Stab(0): orbits {{0}},{sorted(orb_v1)}; argmin={int(mins[0])} unique={unique_min}; "
      f"S9-orbit of minimizer size {len(s9_orbit_of_min)}")
assert nc5, "NC5 control failed: machinery cannot see a unique broken minimizer where one exists"
if nc5:
    print("NEGATIVE_CONTROL_CAUGHT: non-transitive H=Stab(0) control yields computed unique "
          "non-symmetric minimizer (vertex 0) — outcome (c) is reachable by this machinery; "
          "its absence on the scene is a property of the transitive S9xS11xS13 action")
