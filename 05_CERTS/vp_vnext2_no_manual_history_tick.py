#!/usr/bin/env python3
"""vp_vnext2_no_manual_history_tick - reject a hand-completed history tick U_n."""
import pathlib, re, sys
if hasattr(sys.stdout, "reconfigure"): sys.stdout.reconfigure(encoding="utf-8")
ROOT = pathlib.Path(__file__).resolve().parents[1]
BOOKS = ROOT / "01_BOOKS"
FORBIDDEN = ['manual u_n block completion', 'arbitrary complement dynamics', 'u_n block completion by hand']
...
def main() -> int:
    ...
    assert hits('a manual U_n block completion'), "control: a planted overclaim must be caught"
    print("FAIL_OVERCLAIM_CAUGHT  a planted overclaim is caught.")
    assert not hits("we do not claim " + 'a manual U_n block completion'), "control: a negated disclaimer must NOT be flagged"
    print("PASS_NEGATED_DISCLAIMER_ALLOWED  an honest negated disclaimer is NOT flagged.")
    print('PASS_VNEXT2_NO_MANUAL_HISTORY_TICK')
    return 0
