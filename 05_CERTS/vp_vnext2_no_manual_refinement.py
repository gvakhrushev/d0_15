#!/usr/bin/env python3
"""vp_vnext2_no_manual_refinement - reject a hand-selected refinement family."""
import pathlib, re, sys
if hasattr(sys.stdout, "reconfigure"): sys.stdout.reconfigure(encoding="utf-8")
ROOT = pathlib.Path(__file__).resolve().parents[1]
BOOKS = ROOT / "01_BOOKS"
FORBIDDEN = ['chosen refinement family', 'all-walks chosen because', 'non-backtracking is more physical', 'chosen path family', 'selected history language']
...
def main() -> int:
    ...
    assert hits('a chosen refinement family'), "control: a planted overclaim must be caught"
    print("FAIL_OVERCLAIM_CAUGHT  a planted overclaim is caught.")
    assert not hits("we do not claim " + 'a chosen refinement family'), "control: a negated disclaimer must NOT be flagged"
    print("PASS_NEGATED_DISCLAIMER_ALLOWED  an honest negated disclaimer is NOT flagged.")
    print('PASS_VNEXT2_NO_MANUAL_REFINEMENT')
    return 0
