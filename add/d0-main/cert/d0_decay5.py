#!/usr/bin/env python3
"""CHK 19.4.DECAY5 — Exponent-5 self-consistency check (CORE kernel)

HONESTY NOTE: this is a self-consistency CHK, not a falsifiable PASS/FAIL
certificate. The configuration count N is GENERATED as exactly
round(count_scale * R**5) (see run()), so the fitted log-log slope is ~5 by
construction — only integer-rounding noise can move it, and that noise stays
well inside eps^2. It therefore restates its own generator arithmetically and
can never return FAIL on the data it builds. It is NOT external evidence that
the operational choice dimension D_Sigma = 5; it only checks that the log-log
fit recovers the exponent baked into the generator. To promote it to a real
certificate, supply an independently-produced count series #Omega(R) instead
of the R**5 generator. Reported with status_class="CHK" (matches [CHK
19.4.DECAY5] in BOOK IV) so it is not counted as empirical verification.
...
def main() -> int:
    ...
    proto = {
        ...
        "title": str(P(prot, "title", "Exponent-5 self-consistency check (CHK)")),
    ...
    rep.status_class = "CHK"  # slope recovered from a self-generated R**5 series; cannot FAIL
    rep.add("PROTO_LOADED", True, protocol=args.protocol)

    status, data = run(proto)

    print("=== Exponent-5 self-consistency check (CHK 19.4.DECAY5) ===")
    print(f"phi   = {PHI:.15f}")
    ...
    print("status_class = CHK (slope of a self-generated R^5 series; cannot FAIL — not empirical verification)")
