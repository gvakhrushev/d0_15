...
*The through-line, stated once, from one operational verification criterion to the constants of physics. Every claim
...
The remaining regress is now closed one floor below that formulation. Start with the operational
meaning of a verifiable theory: observational states leave records; at least two registered lines
can compare those records; and every line must return the same equality/difference truth under
every possible external catalogue. Lean then proves three facts that were previously stated as
methodological motivation:

```text
verifiability
  → distinguishable alternatives
  → injective persistent record
  → repeated comparison against one public truth
  → catalogue invariance (class M1).
```

The crucial step is a contradiction, not a naming convention. If two distinguishable states leave
the same record, comparison of that one record must say both “same” and “different”. If a necessary
comparison changes with a privileged catalogue, it may work under the privileged choice but it
cannot be independently verified. Thus M1 is the catalogue-free face of public correctness, rather
than a second physical postulate placed above it.

Operational MDL is now exact at this layer. It counts the semantic image of active functions, not
the words used for them. Every complete vocabulary covers exactly three roles — distinction,
retention and comparison — every bijective renaming is equivalent, and an explicit fourth synonym
is removable without changing capability. Separate physical boxes are not implied: one device may
implement several functions, but none of the functions can disappear.

This also makes the universal claim precise. An empirical theory is in the verifiable class when a
correct protocol represents its canonical observational quotient. Such a theory embeds in its full
catalogue/test/outcome signature space; on the protocol generated by the comparison itself, the
empirical quotient is equivalent to the verified state carrier. The theorem is universal over
theories that supply this operational representation. Claiming that a particular external physical
formalism supplies it remains a checkable representation obligation, exactly as in T50–T52.

When that obligation maps verification lines into the concrete Boolean observation carrier, the
new necessity theorem composes with the existing detector chain: class M1 gives catalogue freedom
(T50–T51), catalogue freedom gives history invariance/current-data factorization (T46–T48),
primitive profiles exhaust to membership or value (T49), and the resulting capability count seals
the port-power bound (T45). The new theorem supplies the missing reason M1 applies to a verified
line; it does not manufacture the physical representation map.

There is a second, less obvious consequence. Independent lines are different registered processes
but correctness makes their full outcome tables identical. Therefore the detector-result layer
cannot injectively retain line identity. Any faithful joint representation `(outcome, memory)`
forces the memory coordinate itself to be injective; on finite carriers its capacity is at least
the number of lines. This derives the detector/archive split from verification itself: results must
agree, while provenance must remain distinguishable. A one-state archive fails constructively.
History is therefore excluded from the primitive current-data readout for a reason, yet it cannot
be destroyed globally — it moves to the provenance archive.
...
2. **Machine-checked core.** The load-bearing steps are not prose. `lake build D0.All` is
   green with **0 `sorry`** across 4558 jobs; the certificates are required to be able to fail.
...
The role-level regress beneath M1 is now closed by T53–T57. The sharp remaining squeeze is
quantitative: `N < D` loses distinguishability (closed), while the general `N > D` branch still
needs the separate significance-catalogue/canonization theorem outside the verified three-role
interface. That is the edge of the theory, stated plainly, because a theory worth anything tells
you exactly where to attack it.
