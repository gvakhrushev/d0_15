...
# ---------- 4b. GUARD-GRADE NEGATIVE CONTROL: a T genuinely COUPLED to the spatial lattice ----------
# Can-FAIL the CONCLUSION: build a HYPOTHETICAL flow in which the time operator's
# contraction is fed INTO the zone-9 label space as per-vertex weights w_k
# (distinct rationals, k=0..8) instead of the owned uniform scalar φ⁻¹·I.
# Iterate the flow from the SYMMETRIC uniform state (exact Fraction arithmetic,
# no float): the attractor concentrates on the unique max-weight vertex —
# i.e. a lattice-coupled T DOES select a unique within-zone label.  If this
# computed control failed to select, the scene's NO-SELECTOR verdict would be
# untestable by this probe and the cert EXITS 1.  (The coupled weights are, of
# course, NOT owned — supplying them is exactly the relocated catalog.)
coupled_w = [F(9 - k, 10) for k in range(9)]      # distinct per-vertex contraction weights
coupled_diag = [(w, F(0)) for w in coupled_w]     # as ℚ(φ) elements (rational part only)
# (i) coupled operator is NON-scalar within-zone => does NOT commute with S9 (same code path)
coupled_commutes = commutes_with_all_perms(coupled_diag, 9)
check("NEG-CONTROL-4b: lattice-coupled T is non-scalar (breaks S9 commutation)",
      not coupled_commutes, "distinct diagonal => fails the same commutation test the owned tick passes")
assert not coupled_commutes, "coupled control unexpectedly commutes — control construction broken"
# (ii) iterate the coupled flow 64 ticks from the uniform state, exact Fractions
state = [F(1)] * 9
for _ in range(64):
    state = [coupled_w[k] * state[k] for k in range(9)]
top = max(state)
dominant = [k for k in range(9) if state[k] == top]
share = top / sum(state)
flow_selects_unique = (len(dominant) == 1)
check("NEG-CONTROL-4b: coupled flow attractor selects a UNIQUE vertex (computed)",
      flow_selects_unique and share > F(99, 100),
      f"argmax after 64 ticks = vertex {dominant[0] if flow_selects_unique else dominant}, "
      f"amplitude share = {float(share):.6f} (>0.99)")
assert flow_selects_unique and share > F(99, 100), \
    "coupled-flow control failed to select — the probe could never print SELECTOR-FOUND"
if flow_selects_unique:
    print(f"NEGATIVE_CONTROL_CAUGHT: lattice-coupled T selects vertex {dominant[0]} with computed "
          f"share {float(share):.6f} after 64 exact ticks — selection IS reachable by this probe; "
          f"the owned uniform-scalar tick simply never reaches it (and the coupling is not owned)")
