...
The catalog extension_primitives.csv is, since motion A(d) of the CEMENT
...
BUNDLES.  The minimality/independence claim therefore has two layers:
  ...
  (2) INDEPENDENCE layer: the derive-relation on the catalog is EXACTLY the
      registered edge ledger (extension_primitive_derive_edges.csv, each
...
def load():
    P = list(csv.DictReader((ROOT / "02_REGISTRY/frontier/extension_primitives.csv").open(encoding="utf-8", newline="")))
    M = list(csv.reader((ROOT / "02_REGISTRY/frontier/extension_primitive_independence_matrix.csv").open(encoding="utf-8", newline="")))
    raw = list(csv.reader((ROOT / "02_REGISTRY/frontier/extension_primitive_derive_edges.csv").open(encoding="utf-8", newline="")))
    ...
    for r in raw[1:]:
        ...
        if section == "edges":
            ...
            excluded[r[0]] = r[1]
    pairs = list(csv.DictReader((ROOT / "02_REGISTRY/frontier/extension_primitive_pair_justifications.csv").open(encoding="utf-8", newline="")))
    regs = list(csv.reader((ROOT / "02_REGISTRY/claims.csv").open(encoding="utf-8", newline="")))
