...
BOOKS = ROOT / "01_BOOKS"
CSV = ROOT / "02_REGISTRY" / "claims.csv"
ROSETTA = ROOT / "02_REGISTRY" / "VOCABULARY.md"
...
def main() -> int:
    ...
    missing_new = [c for c in pass_certs if not (ROOT / "04_CERTIFICATES" / c).exists()]
    assert not missing_new, f"this pass cites cert files absent on disk: {missing_new}"
    print(f"PASS_NEW_CERT_REFS_RESOLVE  all {len(pass_certs)} cert files this pass cites resolve in 04_CERTIFICATES/ "
