...
rewrite (which would break the phason-sync guards and risk formula corruption across 342 fragments):
  ...
  - the master Rosetta crosswalk (02_REGISTRY/VOCABULARY.md) exists and
...
def main() -> int:
    ...
    for ref in re.findall(r"02_REGISTRY/[A-Za-z0-9_./-]+", prose):
        target = ROOT / ref
        assert target.exists(), f"broken Rosetta cross-reference in books: {ref}"
    print("PASS_NO_BROKEN_XREF  every 02_REGISTRY/... reference in the books resolves on disk")
