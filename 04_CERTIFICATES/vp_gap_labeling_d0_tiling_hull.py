...
STATUS string and canonical 02_REGISTRY/gap_labels.json contract (same keys) so all existing references
...
Fricke-Vogt): 04_CERTIFICATES/vp_fibonacci_gap_labeling_owner_edge.py (D0-GAP-LABELING-OWNER-EDGE-001).
"""
...
def run_certificate() -> None:
    ...
    # 4. Verify the canonical registry artifact; do not mutate the repository.
    root = Path(__file__).resolve().parents[1]
    canonical = root / "02_REGISTRY" / "gap_labels.json"
    frozen = json.loads(canonical.read_text(encoding="utf-8"))
    assert frozen == labels, "canonical 02_REGISTRY/gap_labels.json is stale relative to the computed spectrum"
    print("    Canonical 02_REGISTRY/gap_labels.json matches recomputation: PASS")
