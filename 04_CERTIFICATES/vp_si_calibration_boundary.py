...
CERT_DIR = ROOT / "04_CERTIFICATES"

from vp_cosmology_entropy_flow_likelihood import locked_core_params, run_optimization_checks
from vp_spectral_action_eh_coefficient import heat_trace_ladder_report
...
def main() -> int:
    locked = locked_core_params()
    locked["guardrails"] = run_optimization_checks()
    ...
    heat_report = heat_trace_ladder_report()
    a0 = float(heat_report["weighted_trace_a0"])
    a2 = float(heat_report["a2_proxy"])
    ...
    output_path(__file__, "bridge_schema.json").write_text(
        json.dumps(bridge_schema, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    output_path(__file__, "si_calibration_boundary_report.json").write_text(
        json.dumps(
            {
                "status": "BRIDGE_CALIBRATION",
