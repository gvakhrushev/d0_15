...
def main() -> int:
    ...
    j=json.load((ROOT/"04_CERTIFICATES/data/raw_input_provenance.json").open(encoding="utf-8"))
    assert j["stipulation"] is False and len(j["raw_objects"])>=5
    rows=vcsv("raw_object_map.csv"); assert all(r["lean_constant"].startswith("D0.") for r in rows)
