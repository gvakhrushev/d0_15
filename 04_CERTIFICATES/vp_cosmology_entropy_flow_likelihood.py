...
def export_artifacts(core_shape: np.ndarray, guardrails: dict[str, bool]) -> None:
    with output_path(__file__, "D0_SDE_Core_Shape.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    output_path(__file__, "locked_core_params.json").write_text(
        json.dumps(metadata, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    output_path(__file__, "bridge_params_schema.json").write_text(
