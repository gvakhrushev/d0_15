...
def lean(path):
    return (ROOT/"03_FORMALIZATION"/path).read_text(encoding="utf-8")
def vcsv(name):
    return list(csv.DictReader((ROOT/"04_CERTIFICATES/data"/name).open(encoding="utf-8",newline="")))
...
def main() -> int:
    ...
    rs,H=None,None
    reg=list(csv.reader((ROOT/"02_REGISTRY/claims.csv").open(encoding="utf-8",newline=""))); H=reg[0]
    ...
    cl=vcsv("alpha_analytic_candidate_class.csv"); assert any("LINDEMANN" in r["external_assumption"].upper() for r in cl)
