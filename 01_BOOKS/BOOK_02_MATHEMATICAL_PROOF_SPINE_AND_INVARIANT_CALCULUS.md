...
**[D0-X5 alpha boundary].** Alpha stays OUTSIDE the five contracts (5-layer boundary); D0-X5 does not solve the residue — the a=3 candidate log-Cesàro `1/(3 log φ) ≠ μ₂` is external (Lindemann), never CORE (`D0-X5-ALPHA-BOUNDARY-001`). Typed operator morphisms only; no trace/det proxy intertwiner.
...
**[THE] 02.7A.1 (Quadratic response is forced — Gleason loophole closure).** The self-reading map lives on a 2D retained/archive phase pair and must preserve the boundary area it reads, since area is the only catalog-free measure available to it. An area-preserving readout on a 2D phase cell admits no admissible *linear* scalar response: a linear functional on the retained amplitude bypasses the archive coordinate and so fails to be a function of the area the carrier actually records. The owned uniqueness premise inside the phase-quadratic class is quarter-turn phase blindness (`QuarterTurnInvariant`). Generic area preservation does not force \(x^2+y^2\); `BornAreaPreservationNoGo` (`D0-BORN-AREA-PRESERVATION-INSUFFICIENT-NOGO-001`) is the negative control. Gleason's theorem leaves a loophole at low dimension, and that loophole is not closed by area preservation alone [^b02-14]. The quadratic readout is then *used* in proof form in §02.8 ([THE] 2.12, Quadratic Readout Form); §02.7A supplies the *why*.
...
**[OBSERVATION — regularized model] Polynomial relaxation via the Lyapunov functional $V_k$.**  For the class $\mathbf{NP}_{D0}$ — tasks given by a finite predicate $\chi(x)$ on a constructive type and realized on the finite levels $V_k$ of the inverse system — if each level $k$ carries a global heat-trace functional $V_k$ (a Lyapunov function) under which coarse-graining is a monotone descent $V_k\downarrow$ that separates admissible solutions by a spectral gap $\Delta_k>0$, then search reduces to spectral relaxation, executable in $\mathrm{poly}(|\mathrm{Rep}(x)|)$ per fixed level [^b02-28].

**[OBSERVATION — regularized model, NOT the P-vs-NP problem] Constructive-phase collapse $\mathbf{NP}_{D0}\subseteq\mathbf{P}_{D0}$.**  If $\Delta_k\ge\Delta>0$ holds across the whole level family (a spectral phase without criticality), then $\mathbf{NP}_{D0}$ admits a polynomial solver on the physical D0-geometry carrier, i.e. $\mathbf{NP}_{D0}\subseteq\mathbf{P}_{D0}$.  *This is not, and does not claim to be, the ZFC $\mathbf P\overset?=\mathbf{NP}$ question*: it is a statement about the regularized computational model with a fixed UV cutoff and holonomic memory topology, where every task names a test [^b02-29].  The disclaimer is part of the claim — without the fixed cutoff and the test requirement, the statement would not be a corpus statement at all.  In particular the conclusion is conditional on the uniform-gap hypothesis $\Delta_k\ge\Delta>0$ holding across the whole tower; that hypothesis is an assumption of the cell, not a forced output, so nothing here bears on the classical P-vs-NP separation.
...
- depth `\varphi^{-17}=\varphi^{-5}\!\cdot\varphi^{-12}` — the seam `\xi_5` times the electroweak transport; the composition now carries a toral LAW (`D0-TORAL-COMPOSITION-SEVENTEEN-001`, clean axioms): both factors are return defects of the same toral automorphism — `\xi_5=\varphi^5-11` and `\varphi^{-12}=322-\varphi^{12}=L_{12}-\varphi^{12}` — and the seventeenth return factors through the fifth and twelfth, `L_{17}=L_{12}L_5+L_7` with the `+L_7` correction forced by the odd return's orientation reversal (`\det T^5=-1`); the depth sub-leg itself stays the named open obligation (the seam↔return identification is the primitive, door 1 the live rival);
...
**[Iter22 eight-front]** **D0-DIXMIER-FESHBACH-FINITE-HEATTRACE-001 (finite NO-GO, CERT-CLOSED).** The Feshbach–Schur archive block is a *finite* multiset of eigenvalues (dim 30), so its heat trace `Θ(s) = Σ_i exp(-s·λ_i)` is entire and its Maclaurin expansion is a power series indexed by `k ≥ 0` only, with `a_k = (Σ_i (-λ_i)^k)/k!`. Three decidable/`ring` facts are owned (Lean `D0.Spectral.DixmierFeshbachFiniteHeatTrace`, cert `vp_dixmier_feshbach_finite_heattrace.py`): (i) there is **no `1/s` pole** — the residue-slot coefficient `a_{-1}` is structurally `0` (a ℕ-indexed series has no negative index), equivalently `s·Θ(s) = O(s) → 0`; (ii) the value at `s = 0` is the **dimension**, `Θ(0) = a_0 = card = 30` (and emphatically *not* the depth-2 archive moment `μ₂ = 12288/5`); (iii) the **contrast that forces an infinite spectrum**: the truncated tower `Σ_{k<N} x^k = (1−x^N)/(1−x)` stays finite as `x→1` with value exactly `N` (telescoping identity proved at `N=30`), whereas the `1/s` pole with residue `c_{-1}=1` is the *infinite* geometric tower `Σ_{k≥1} e^{-sk}` — absent from every finite truncation. This is an anti-numerology guard: it CANNOT and does not close the Dixmier/Wodzicki residue on the genuinely infinite spectral object, nor the residue→`Δ_α` normalization, which remain the external owner edge `D0-DIXMIER-RESIDUE-OWNER-001` (`ASSUMP-DIXMIER-TRACE`). No measured/CODATA `α` enters.

**[Static-to-dynamics pointer].** The same retained/traced Feshbach–Schur complement used here for the pole / effective-transfer coefficient owners is also the finite time-delay owner: in BOOK_06 §06.8 the Neumann index `k` of the archive resolvent `(I - Q U Q)^{-1}=Σ_k (Q U Q)^k` is cert-closed as the discrete time tick (`D0-FESHBACH-SCHUR-TIME-DELAY-OWNER-001`, combined `D0-STATIC-TO-DYNAMICS-OWNER-001`).## 02.14 Carrier, coefficient and gravity proof blocks
...
Both parameters are read off the toral operator \(T = [[0,1],[1,-1]]\) without using \(|V_{11}|\): \(\Delta = \exp\mathrm{Fix}(T^3) = 2\) with \(\mathrm{Fix}(T^3) \cong (\mathbb Z/2)^2\), \(|\Omega_8| = 2\,\#\mathrm{Fix}_3 = 8\), and \(m = \#\mathrm{Fix}_5 = 11\) is the first return count exceeding \(|\Omega_8|\); the product \(\#\mathrm{Fix}_3\cdot\#\mathrm{Fix}_5 = 44\) is the return modulus (`D0-TORAL-ORIGIN-OF-QT-NONCIRCULAR-001`, `D0-TORAL-ORIGIN-OF-P2-001`). The threshold is derived from M1-minimality (\(|V| \ge |\Omega_8|+1 = 9 \implies n^*=5, m=11\), `D0-M1-MINIMALITY-THRESHOLD-FORCING-001`). The golden class itself is globally forced across \(\mathrm{GL}_2(\mathbb Z)\) by sterility of returns 1 and 2, which is equivalent to transitive generation dynamics on \(\mathbb P^1(\mathbb F_2)\) (`D0-GLOBAL-RETURN-STERILITY-GOLDEN-FORCING-001`, `D0-TRANSITIVE-DEFECT-AND-COLLISION-UNIQUENESS-001`). The same tuple is returned by the typed preparation register at \((q,r) = (2,2)\), where \(q=2, \Delta=2, r=2\) are uniquely forced by arithmetic centering, collision balance and verification non-solipsism (`D0-TWO-CHANNEL-SCENE-WELD-001`, `D0-STEP-TWO-AND-LINE-COUNT-NOGO-001`, `D0-TRANSITIVE-DEFECT-AND-COLLISION-UNIQUENESS-001`), and representation uniqueness holds by Cartan Lie classification, projective ray counting, and line parity (`D0-M1-REPRESENTATION-UNIQUENESS-001`); the \(\mathrm{GL}_2(\mathbb F_2)\) functor on \(\mathbb F_2^2\) canonically unifies the three zones with line-swap orbits and the period-3 toral return orbit (`D0-THREE-ZONES-SEED-ORBIT-IDENTIFICATION-001`); see §01.22a for the complete derivation and the recorded no-go on Lucas coincidence as a stand-alone selector.
...
- **SM-facing Gauge Decomposition**:
  ...
  - `D0.Gauge.AlbertSMProjection`: dimension and charge alignment of the Albert algebra $J_3(\mathbb{O})$ with $\mathfrak{su}(3) \times \mathfrak{su}(2) \times \mathfrak{u}(1)$ ($\dim = 12 < 52$) and Majorana $B - L$ hypercharge matching (`D0-ALBERT-SM-GAUGE-DIMENSION-ALIGNMENT-001`). This is not a derived automorphism algebra and not a proved Lie-subgroup embedding.
