...
Both parameters are read off the toral operator \(T = [[0,1],[1,-1]]\) without using \(|V_{11}|\): \(\Delta = \exp\mathrm{Fix}(T^3) = 2\) with \(\mathrm{Fix}(T^3) \cong (\mathbb Z/2)^2\), \(|\Omega_8| = 2\,\#\mathrm{Fix}_3 = 8\), and \(m = \#\mathrm{Fix}_5 = 11\) is the first return count exceeding \(|\Omega_8|\); the product \(\#\mathrm{Fix}_3\cdot\#\mathrm{Fix}_5 = 44\) is the return modulus (`D0-TORAL-ORIGIN-OF-QT-NONCIRCULAR-001`, `D0-TORAL-ORIGIN-OF-P2-001`). The threshold is derived from M1-minimality (\(|V| \ge |\Omega_8|+1 = 9 \implies n^*=5, m=11\), `D0-M1-MINIMALITY-THRESHOLD-FORCING-001`). The golden class itself is globally forced across \(\mathrm{GL}_2(\mathbb Z)\) by sterility of returns 1 and 2 (`D0-GLOBAL-RETURN-STERILITY-GOLDEN-FORCING-001`). The same tuple is returned by the typed preparation register at \((q,r) = (2,2)\), where \(q=2, \Delta=2, r=2\) are uniquely forced by arithmetic centering and verification non-solipsism (`D0-TWO-CHANNEL-SCENE-WELD-001`, `D0-STEP-TWO-AND-LINE-COUNT-NOGO-001`), and the \(\mathrm{GL}_2(\mathbb F_2)\) functor on \(\mathbb F_2^2\) canonically unifies the three zones with line-swap orbits and the period-3 toral return orbit (`D0-THREE-ZONES-SEED-ORBIT-IDENTIFICATION-001`); see §01.22a for the complete derivation and the recorded no-go on Lucas coincidence as a stand-alone selector.
