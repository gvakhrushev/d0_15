...
[SHADOW] **The "second-order/`π₀`-phase moment" now has a precise finite meaning (`D0-DELTA-ALPHA-MOMENT-001`, sharpens the owner; does not close it).** Substituting `δ₀ = ½φ⁻³` collapses the algebraic writing into a polynomial in the single rank-3 archive unit `u = φ⁻³` (the per-excursion archive weight):
