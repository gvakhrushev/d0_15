...
| `D0-BOUND-001` | D0-RESOLUTION-EXACT | q_mass and boundary curvature residual | Quotient/Resolution | Do not fit q_res with arbitrary scalar factor or reopen an infinite residual spiral. | Boundary residual cannot be produced below I12 by admissible cell. |
...
```math
P_N(a)=F_N(\Pi_a)=\frac{\operatorname{Tr}(\Pi_a R_N)}{\operatorname{Tr}(R_N)}.
```
