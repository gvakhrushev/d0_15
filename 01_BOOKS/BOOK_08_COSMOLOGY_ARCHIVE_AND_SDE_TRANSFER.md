...
**Dark-energy connection (HYP, not BRIDGE — awaits DESI DR3).** If the archive phason carries a slow-rolling pressure, it reads as an evolving dark-energy equation of state `w(z)`. The DESI DR2 analysis prefers a CPL evolving `w` over `ΛCDM` at `~3.1σ` (K. Lodha et al., arXiv:2503.14743). This is *suggestive* of "evolving `w` = archive pressure", **but the mechanism is not derived**: no numerical `w(z)` has been computed from the phason model (the 30-dimensional kernel → `w(z)` map is unwritten). Status **HYP, not BRIDGE**, pending a phason-derived `w(z)` and DESI DR3 — recorded so a survey hint is not mistaken for a forced prediction.
