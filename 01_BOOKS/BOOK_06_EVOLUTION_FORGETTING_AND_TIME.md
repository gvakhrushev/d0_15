...
**Internal Lucas–Voronoi Markov partition (`D0-LUCAS-VORONOI-MARKOV-PARTITION-001`).** The toral time generator `T = \begin{psmallmatrix}0&1\\1&-1\end{psmallmatrix}` is read not via an *imported* Adler–Weiss theorem but through an **internal** partition built from integer Lucas-trace data: `Tr(Tⁿ) = (−1)ⁿ Lₙ` gives explicit boundary cuts `L₂ = 3, L₃ = 4, L₅ = 11`; the map is volume-preserving (`det(Tⁿ)² = 1`) and **κ-stable** (the Pisot contraction `|ψ| < 1` forces finite refinement) — Lean `D0.Geometry.LucasVoronoiMarkovPartition`, cert `vp_lucas_voronoi_markov_partition.py`. This is OPERATOR-SCAFFOLD-CERTIFIED: the explicit Lucas-boundary partition + κ-stability are owned; the exact Markov property is now realized internally via the golden `β = φ` natural extension (`D0-ADLER-WEISS-INTERNAL-CONSTRUCTION-001`, CERT-CLOSED — see the Iter25 closure below; the metric flat-torus embedding stays external), while the former raw-space claim `Torus² ≅` golden SFT is now closed **negatively** (`D0-TORAL-TIME-MARKOV-CONJUGACY-001`, NO-GO): the torus is connected and nontrivial whereas the raw SFT is totally disconnected, so no homeomorphism — hence no topological conjugacy — can exist. The D0-internal profinite-code conjugacy is already closed (`D0-PHI-STURMIAN-PROFINITE-CODE-CONJUGACY-001`); classical Adler–Weiss stays the cited external owner `D0-ADLER-WEISS-PARTITION-OWNER-001`, not re-derived.

**[Iter23 — the partition's symbolic owner, pinned internally and exactly (`vp_adler_weiss_symbolic_owner.py`).** The one piece the Adler–Weiss internal-construction target still needs is *geometric* (the metric cells); the *symbolic* model behind them can, however, be named exactly, and it is. In exact integer arithmetic, `T = \begin{psmallmatrix}0&1\\1&-1\end{psmallmatrix}` is `GL(2,ℤ)`-conjugate to `−S`, where `S = \begin{psmallmatrix}1&1\\1&0\end{psmallmatrix}` is the incidence matrix of the **Fibonacci substitution** `σ: 0 ↦ 01, 1 ↦ 0` (equivalently the golden-mean SFT matrix `N_τ`), the conjugator being the *minimal* unimodular `U = \begin{psmallmatrix}0&1\\-1&0\end{psmallmatrix}` (the 90° rotation, `det +1`): `U T U^{-1} = −S`, verified exactly. Hence the Adler–Weiss Markov partition of `T` is the geometric (Rauzy) realization of the Fibonacci substitution, **twisted by orientation** — `−S`, not `+S` (`tr T = −1 = tr(−S) ≠ tr S = 1`), which is precisely the orientation obstruction the periodic-point count exposes below. This *sharpens* the `D0-ADLER-WEISS-INTERNAL-CONSTRUCTION-001` missing statement from "find the symbolic model of the partition" to "realize the Rauzy parallelogram cells of the (now-known, orientation-twisted Fibonacci) substitution," a classical metric construction. The symbolic leg is owned internally.

**[Iter25 — the internal Markov realization is now CERT-CLOSED (`D0-ADLER-WEISS-INTERNAL-CONSTRUCTION-001`, cert `vp_adler_weiss_internal_markov.py`, Lean `D0.VNext2.AdlerWeissInternalMarkov`).** The exact 2-cell Markov partition is realized *internally* through the M1-forced golden `β = φ` natural extension, closing the symbolic-dynamical content of the target. The digit map `d(x) = ⌊φx⌋` on `[0,1)`, cut at the golden split `1/φ` (which satisfies the M1-forced `1/φ + 1/φ² = 1`, not a tuned threshold), gives cells `R₀ = [0, 1/φ)` and `R₁ = [1/φ, 1)`. The Markov property is *proven symbolically* (exact in `ℤ[φ]`, not simulated): `x ∈ R₁ ⟹ φx − 1 ∈ [0, 1/φ) = R₀` exactly (since `φ − 1 = 1/φ`), so digit `1` forces the successor digit `0` — the transition `1 → 1` is **forbidden**; and `x ∈ R₀ ⟹ φx ∈ [0,1) = R₀ ∪ R₁`, so digit `0 → {0,1}`. The transition matrix is therefore exactly `[[1,1],[1,0]]` — the golden SFT — which *is* the Markov property `T(int Pᵢ) ⊆ ⋃ int Pⱼ`, transported through the owned orientation-twisted conjugacy `T ∼ −M_φ` (Lean-verified `conjugacy_T_negMphi`, `det U = +1`, rc = 0). The Parry (maximal-entropy) cell measures are exactly `(φ², 1)/(φ²+1) = (0.7236, 0.2764)` and the topological entropy is `log φ` (also confirmed by a 3·10⁵-step natural-extension simulation: `1→1` empty, frequencies `0.723/0.277`). **Honest scope:** what is now internal is the *symbolic-dynamical* realization (the exact Markov property, golden SFT, Parry data) in the M1-native `β = φ` coordinates; the explicit flat-torus parallelogram (Rauzy) *embedding* with affine cell polygons, and the smooth/measure conjugacy, remain the external classical owner `D0-ADLER-WEISS-PARTITION-OWNER-001`. Two naive metric attempts (axis-aligned unstable strips; eigen-coordinate boxes) provably failed the 2-D torus tiling — the `β = φ` natural extension is the correct internal route.

**[Iter22 eight-front]** The shift-equivalence question for the D0 toral time operator now has a precisely-scoped internal closure (D0-WILLIAMS-SHIFT-EQUIVALENCE-OWNER-001, CERT-CLOSED). Over the integers we prove: charpoly(T)=X^2+X-1 and charpoly(M)=X^2-X-1 for the Fibonacci companion M=[[1,1],[1,0]] (from trace/determinant); an explicit GL(2,Z) similarity U*(-T)=M*U with U=[[0,1],[-1,0]] and det U=+1, exhibiting N=-T and M as conjugate over Z; and agreement of the two classical shift-equivalence invariants — equal nonzero spectrum (charpoly(N)=charpoly(M), the Galois sign-flip r<->-r of charpoly(T)) and equal Bowen-Franks group Z^2/(I-A)Z^2 (the Smith normal form of both I-M and I-N is diag(1,1), so both groups are trivial), each witnessed by explicit unimodular reductions P*(I-A)*Q=I. This is exactly the necessary-condition layer: the shift-equivalence invariants agree, but they are necessary not sufficient for conjugacy. The former **raw** topological-conjugacy target is now NO-GO-CLOSED: a connected two-torus cannot be homeomorphic to a totally disconnected raw SFT (`D0.Dynamics.ToralSFTConjugacyNoGo`). Williams strong-shift-equivalence and Adler-Weiss remain relevant to the correctly typed symbolic factor / Markov coding and to boundary-quotient models, not to a homeomorphism `Torus² ≃ raw SFT`. `D0-ADLER-WEISS-INTERNAL-CONSTRUCTION-001` still owns the symbolic-dynamical leg; the metric flat-torus embedding remains with the external owner. The cert `vp_toral_shift_equivalence.py` enforces this boundary with reachable controls rejecting (i) upgrading agreeing invariants to a conjugacy claim, (ii) a wrong companion matrix, and (iii) a non-golden/non-Lucas boundary matrix.

**[Iter23 — the elementary strong-shift-equivalence datum, now internal.** The paragraph above supplies the shift-equivalence *invariants* (equal spectrum, equal Bowen–Franks group) but explicitly leaves the actual Williams `(R,S)` strong-shift-equivalence matrices *not constructed internally*. We now construct an explicit elementary `(R,S)` step, in exact integer `2×2` arithmetic. Using the nonnegative golden `φ²` SFT matrix `A = N_τ² = [[1,1],[1,2]]` (with `N_τ=[[0,1],[1,1]]`, `spec = {φ², φ⁻²}`), the nonnegative integer pair `R=[[1,0],[1,1]]`, `S=[[1,1],[0,1]]` gives `A = R S` and the elementary-equivalence partner `B = S R = [[2,1],[1,1]]`, with the Williams lag-1 identities `A R = R B` and `S A = B S` verified. This `B` is `GL(2,ℤ)`-conjugate to `T² = [[1,-1],[-1,2]]` (via `U=[[-1,0],[1,1]]`, `det=−1`), which — together with the unimodular `T ∼ −N_τ` conjugacy `P=[[0,-1],[1,1]]` above (so `T` is the *time-reversed* Fibonacci automorphism, `spec(T)=−spec(N_τ)`) — ties the elementary `(R,S)` step to the toral map's own square. This is a genuine internal advance on the **algebraic** leg: the `(R,S)` datum that was flagged "not constructed" now exists (cert `vp_toral_shift_equivalence_supplied.py`, failable). It remains one elementary step at the `φ²` level, **not** the full lag-`L` Williams chain, and it does **not** by itself establish topological conjugacy — that still requires the geometric Adler–Weiss Markov partition (whose owner `D0-ADLER-WEISS-INTERNAL-CONSTRUCTION-001` is since CERT-CLOSED on the symbolic leg, Iter25 above — the geometric/metric embedding stays with the external owner `D0-ADLER-WEISS-PARTITION-OWNER-001`), which the seed-Markov NO-GOs below show is not forced by the invariants alone. Owner `D0-TORAL-TIME-MARKOV-CONJUGACY-001` is now NO-GO for the raw-space formulation. The geometric partition leg remains meaningful only for the positive replacement: a finite-to-one symbolic factor/semiconjugacy or a boundary-quotient symbolic model.
