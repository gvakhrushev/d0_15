...
**Verified closure protocol (Iteration 22, `D0-VERIFIED-CLOSURE-PROTOCOL-001`, CERT-CLOSED).** D0 closure campaigns now use a mandatory verify-then-build protocol (`04_VERIFICATION/VERIFIED_CLOSURE_PROTOCOL.md`): a grounded scout verdict (`CERT-CLOSABLE` / `NO-GO-CLOSABLE` / `PARTIAL-CLOSABLE` / `NOT-CLOSABLE` / `DUPLICATE-ALREADY-OWNED`) before any Lean is written, then Lean/cert implementation with reachable negative controls, book/registry integration, and a full green gate before claim promotion. No proof-target is promoted by status alone. Guard `vp_verified_closure_protocol.py`.## 05.7 One promotion process
