...
- `MECH-LIMIT` (prose status qualifier; **formerly written `LEM`**): a result whose *mechanism* is established and finite-certified, but which is held **below** `THE` because a **limit or external-mechanism** step remains (a named gap) — a conditional result. Used in prose as "status `MECH-LIMIT`, not `THE`" (e.g. the Feshbach generative-dynamics continuum step `D0-GENERATIVE-DYNAMICS-001`; the time↔flavor single-modular-object identification `D0-MODULAR-TIME-FLAVOR-001`). **Naming note:** this qualifier was written `LEM` (for *limit/external mechanism*) in earlier iterations; it is renamed `MECH-LIMIT` to remove the collision with the **Law of Excluded Middle**. It is **distinct** from `LEM` = **Lemma** in the standard `DEF/LEM/THE` proof-object taxonomy, which is unchanged. `MECH-LIMIT` is a prose qualifier, **not** a canonical `release_status` enum value (the carrying claim's release_status stays `CERT-CLOSED`/`BRIDGE-*` with its named gap).
...
The criterion of value is the **registry closure**, never the mere presence of something in a prior corpus. Two failure modes this rule blocks: (i) *re-deriving* what the registry already owns (a duplicate, or a weaker re-derivation that competes with the canonical owner); (ii) the false equation **"it fell out of v14 ⇒ it is valuable ⇒ restore it"** — much of what was dropped was dropped *because the registry closed it better, put it out of core, or marked it no-go*. So before opening any "I derive/return X": (1) grep `theory_status_map.csv` / `CLAIM_TO_LEAN_MAP.csv` for an existing owner — if found, *cross-reference* it, do not duplicate or compete; (2) grep `03_THEORY_MAP/GOLDEN_COVERAGE_LEDGER` — a GOLDEN result may already be present, or be a *definition without a derivation* (so "restoring" it would smuggle in an unproved step). Restore/return only what is **genuinely open AND not better-closed elsewhere AND not a no-go**; everything else is a cross-ref or is left alone. Worked calibration (Iter-17): `m_rest=m₀·W` is GOLDEN but its `W` are *undefined there* and the numeric mass spectrum is a deliberate no-go (`D0-GEN-MASS-001`), so `W` was kept a physical input, **not** restored as core; the golden-split `W_int=W_ext²` is the already-CORE primitive `p+p²=1`, so it was cross-reffed, not re-derived.
