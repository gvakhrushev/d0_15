...
5. **Charged-lepton INDIRECT coefficient origin — open (`D0-LEPTON-INDIRECT-COEFFICIENT-OWNER-001`, PROOF-TARGET).** The direct raw-graph route is no-go'd (`D0-BARE-GRAPH-DECIMAL-NOGO-001`); the only admissible positive route is finite shell torus → finite Green function → ramification/Puiseux branch indices → exact coefficient row. Missing artifact: a finite Green function over the shell torus + the Puiseux/ramification extraction theorem (branch indices = the exact exponent row `(0,1/4,1/3)`) + a branch-index uniqueness certificate + the EFT/IR matching functor. The decimals stay HYP (`D0-LEPTON-002`).

6. **Phason `w(z)` transfer — explicit `w_D0(u)` open (`D0-PHASON-WZ-TRANSFER-OWNER-001`, PROOF-TARGET).** The kernel-only no-go (`D0-PHASON-WZ-KERNEL-ONLY-NOGO-001`) and the EOS-form scaffold (`D0-PHASON-PRESSURE-EOS-SCAFFOLD-001`, `w_D0=p/ρ`) are closed. Missing artifact: the explicit internal `w_D0(u)` / finite `w_N` formula (the phason `ρ(u), p(u)` functions over the archive windows). The redshift `u→z` / CPL reading is passport-only; no survey datum defines the object.
