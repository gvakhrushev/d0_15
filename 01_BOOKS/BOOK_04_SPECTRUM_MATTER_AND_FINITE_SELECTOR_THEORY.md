...
**[Five-primitive: grading + lepton].** The raw commutant Wedderburn structure is now DERIVED — `M₃(ℂ)⊕ℂ⊕ℂ⊕ℂ` (4 blocks, dims 9+1+1+1; the `M₃` block `= 3²` from the diagonal pair-orbit count, not from dim 12 alone) — `D0-RAW-COMMUTANT-WEDDERBURN-001`. On that `M₃` the grading is two-completion (nc `8` vs `12`, `D0-GRADING-NEUTRAL-CURRENT-MAXIMALITY-NOGO-001`); the lepton orbit-keyed selector exists but `2` orbits `< 3` generations (`D0-LEPTON-BRANCH-FIXING-OPERATOR-OWNER-001`, L4).

**[Iter23 — Lane L closed as a NO-GO (`vp_lepton_branch_fixing_operator_nogo.py`, Lean `D0.Extensions.LeptonBranchFixingNoGo`).** The terminal-L4 gap (the bare inequality `2 < 3`) is now a **proven non-constructibility**, machine-checked in Lean against Mathlib. The frozen shell-torus `Ueff = blockdiag(4\text{-cycle}, 3\text{-cycle})` supplies exactly `numBranches = 2` orbit-branches (exponents `1/4 ≠ 1/3`) and has **no fixed point** — so there is no in-carrier third (electron, index-`0`) branch — while `numGenerations = 3` (the `K(9,11,13)` trivial-isotype multiplicity `Tr(T²) = 3`). A branch→generation full-row operator would have to be an injection `Gen(3) ↪ Branch(2)`, a surjection `Branch(2) ↠ Gen(3)`, or a bijection `Branch(2) ≃ Gen(3)`, and **all three are impossible by cardinality** (Lean `decide`). Hence `PRIM-LEPTON-BRANCH-FIXING-OPERATOR` is *not constructible* from the frozen 2-orbit data; the third datum is genuinely external (postulated HYP in `D0-X5-LEPTON-CONTRACT-001`). This flips the claim from PROOF-TARGET to **NO-GO**: the constructibility question is settled (the operator provably cannot be built from scene orbit-data), the mass decimals remain an EFT/IR passport, and no PDG value enters the core.
**[D0-X5 grading + lepton contracts].** The GammaNC contract (model `diag(1,1,-1)`, sig `(2,1)`) gives `N_active = nc(2,1) = 8` RELATIVE to `D0-X5-G` (deletion → 8 vs 12) — `D0-X5-GRADING-CONTRACT-001`; the LeptonBranchFixer contract acts on the full triple `(0,1/4,1/3)` (`D0-X5-LEPTON-CONTRACT-001`), decimals stay EFT/IR passport. Both HYP, never present-core.
...
| φ-quasicrystal gap-module / hull invariant | `D0.Claims.KTheoryGapModule`; `D0.Claims.ktheory_gap_module` (proof-of-record for the rank-2 gap-label module); Delone/FLC/aperiodicity statements are tracked separately |
...
### Why three copies: integer Lucas quantization and the sedenion/Clifford S₃ family layer
...
A further audit now closes the most tempting shortcut negatively.  `D0-SEDENION-SIGNED-BASIS-S3-NOGO-001` exhausts the signed canonical-basis automorphisms of the integer carrier that preserve the common quaternion block.  The Cayley--Dickson basis table has the exact index law $e_i e_j=\pm e_{i\oplus j}$, so the underlying basis permutation must lie in $GL(4,2)$.  Preserving $H=\{0,1,2,3\}$ leaves 576 linear permutations; the four generator signs give 16 lifts each, hence 9216 exhaustive signed candidates.  The certificate checks all 256 basis products for every candidate. Exactly 384 candidates are algebra automorphisms (24 underlying index maps times 16 sign lifts), and **all 384 stabilize each of the three canonical eight-basis carriers individually**.  A map that swaps the quotient generators $4\leftrightarrow8$ really does exchange two blocks, but all 16 of its sign lifts fail multiplicativity.  Therefore the desired nontrivial $S_3$ action is not hidden in a signed permutation of the canonical integer basis.

The positive automorphism layer is now also explicit.  `D0-SEDENION-BROWN-S3-SCALAR-EXTENSION-001` implements the classical Brown generators over the exact scalar extension $\mathbb Q(\sqrt3)$: the order-three map rotates each $(e_i,e_{i+8})$ plane by $2\pi/3$ with coefficients $-1/2$ and $\pm\sqrt3/2$, while the order-two map is $\epsilon(a+be_8)=a-be_8$.  Lean ties the finite structure-constant table back to the repository Cayley--Dickson multiplication, checks multiplicativity on all canonical basis products, and proves the $S_3$ presentation $\psi^3=\epsilon^2=1$, $\epsilon\psi=\psi^2\epsilon$ on the basis.

This also corrects an important carrier mismatch.  The octonion triple used by the fermion-generation construction is
$\{0,1,4,5,8,9,12,13\}$,
$\{0,2,4,6,8,10,12,14\}$,
$\{0,3,4,7,8,11,12,15\}$,
with common quaternion $\{0,4,8,12\}$.  These are now separately machine-checked as multiplicatively closed and stabilized setwise by the Brown $S_3$ generators.  They are **not** the older three $H=\{0,1,2,3\}$-containing canonical carriers used by the signed-monomial audit, so the positive Brown construction and `D0-SEDENION-SIGNED-BASIS-S3-NOGO-001` are complementary rather than contradictory.

The `D0-SEDENION-D0-REPRESENTATION-FUNCTOR-001` representation bridge is now formalized and certified (`CORE-FORMALIZED`, Lean `D0.Algebra.SedenionClifford8`, cert `vp_sedenion_clifford8_car.py`):
1. The complexified sedenion left-multiplication operators $L_1, \dots, L_8$ satisfy the exact Clifford algebra relations $\{L_i, L_j\} = -2\delta_{ij} I_{16}$ for $C\ell(0,8)$.
2. All 256 Clifford monomials are mutually orthogonal under the Frobenius inner product $\operatorname{Tr}(M_A^T M_B) = 16\delta_{AB}$, spanning the complete matrix algebra $\operatorname{Mat}(16, \mathbb{R})$.
3. The four-pair Witt ladder operators $a_j = \frac{1}{2}(-L_j + \mathrm{i} L_{j+4})$ and $a_j^\dagger = \frac{1}{2}(L_j + \mathrm{i} L_{j+4})$ ($j=1,\dots,4$) satisfy the exact fermionic Canonical Anticommutation Relations (CAR): $\{a_j, a_k\} = 0$, $\{a_j^\dagger, a_k^\dagger\} = 0$, $\{a_j, a_k^\dagger\} = \delta_{jk} I_{16}$.
4. Under the Brown $S_3$ automorphism $\psi_3$, the three generation sectors have exact rank 24 in the semi-spinor sector ($I_1^+ \oplus \psi_3(I_1^+) \oplus \psi_3^2(I_1^+)$) and rank 48 in the full generation sector ($\mathcal{S}_1 \oplus \mathcal{S}_2 \oplus \mathcal{S}_3$), establishing strict linear independence of three fermion generations with an untriplicated gauge algebra. This family $S_3$ remains distinct from standard $\operatorname{Spin}(8)$ triality. The legacy three-label scaffold remains a negative control.
...
**[Iter22 eight-front]** **Riemann-Hurwitz branch index (genus-0 consistency, no uniqueness).** The charged-lepton shell-torus cover is assembled from cyclic companion blocks — the 4-cycle C4 (charpoly z⁴−λ) and 3-cycle R3 (charpoly z³−λ) of D0-EDGE-RAMIFICATION-001. The new claim **D0-LEPTON-RIEMANN-HURWITZ-BRANCH-INDEX-001** (CERT-CLOSED) machine-checks three finite facts over ℚ/ℤ: (i) the cyclic relations C4⁴=λ·I and R3³=λ·I; (ii) the ramification index of an n-cycle is n with Puiseux/ramification exponent 1/n, giving the muon entry 1/4 and tau entry 1/3; and (iii) the Riemann-Hurwitz Euler-characteristic balance — for a connected cyclic degree-n cover of ℙ¹ totally ramified over two points (defect 2(n−1)), the genus solves 2−2g = n·2 − 2(n−1) = 2, forcing g=0. Crucially this is a *consistency* identity, NOT a selection principle: the same arithmetic yields genus 0 for n = 2,3,4,5 alike, so Riemann-Hurwitz does **not** single out the pair (4,3) — any uniqueness reading is false. Likewise the branch index does not determine the operator (the independently established NO-GO D0-LEPTON-PUISEUX-UNIQUENESS-OBSTRUCTION-001 exhibits two distinct 4×4 operators sharing index 1/4). No PDG mass enters anywhere; the finite Green resolvent over the full cover and the branch-index uniqueness certificate remain PROOF-TARGET under D0-LEPTON-INDIRECT-COEFFICIENT-OWNER-001.

**[Lepton Green resolvent owned].** The finite shell-torus Green resolvent `G_shell(z)=(I-z*U_eff)^-1` over the cyclic companion cover `U_eff=diag(P4,P3)` (7x7 permutation, order `lcm(4,3)=12`, invertible) exists on a nonempty non-pole domain (`z=0`: `det=1`); `det(I-zU)=(1-z^4)(1-z^3)` so the pole set is finite (`D0-LEPTON-FINITE-GREEN-RESOLVENT-OWNER-001`, CERT-CLOSED, Lean `D0.Matter.LeptonFiniteGreenResolventOwner`). This closes the resolvent checkpoint; canonical branch projectors stay no-go-bounded (`D0-LEPTON-PUISEUX-UNIQUENESS-OBSTRUCTION-001`) and the mass decimals stay external EFT/IR (HYP).## 04.9 Neutrino leakage sector
...
**[CKM overlap underdetermination].** The mismatch matrix is unique *given* fixed bases, but present-core does not force the basis completion: two admissible rational-orthogonal completions (3-4-5 and 5-12-13 rotations) give different Cabibbo overlap invariants `|V_mix_12|^2 = 16/25` vs `144/169` (`D0-CKM-OVERLAP-UNDERDETERMINATION-NOGO-001`, Lean `D0.Matter.CKMOverlapUnderdeterminationNoGo`). The overlap is underdetermined by present-core; a canonical selector (symmetry-breaking / modulus-fixing) is a new primitive, not a present-core theorem.

**[CKM/PMNS dichotomy from the derived generation algebra `⟨D_zone, S_zone⟩` — qualitative, downstream of F6].** The generation matrix algebra is `M₃(ℂ) = Alg*(D_zone, S_zone)` on the three zone-lines, with `D_zone = diag(24,22,20)` (degree frame) and `S_zone[i,j] = √(n_i n_j)` the zone-transfer, sharing the eigenvector `v = √n` (`D_zone − S_zone = 33·I − v·vᵀ`, `|v|² = 33`) while `[D_zone, S_zone] ≠ 0` — a single derived algebra with structural, nonzero family mixing. Read in the two Yukawa frames it gives a **dichotomy**: *misaligned* frames yield **large** (PMNS-scale) angles and *aligned* frames yield **small** (CKM-scale) angles, with exact-zero mixing as the degenerate negative control. This is recorded as a *qualitative* structural observation only — the exact-angle attempts remain open and downstream of the F6 Higgs/Yukawa selector, not a closed prediction.

**[The systematic 3↔2 generation tension.]** One obstruction recurs identically in three structures built from the same data: the degree frame forces a clean geometric **3** (`g₂₄, g₂₂, g₂₀`), while the `(ℤ/44)*` window classes and the lepton cycle-orbits (`blockdiag(4-cycle, 3-cycle)`) each split **2**. So `PRIM-LEPTON-BRANCH-FIXING-OPERATOR` and the window `MECH-LIMIT-D` are the *same* open gap seen twice — a missing canonical reconciliation `π : 𝒢_zone^{(3)} → {window classes}^{(2)}` with the third generation as `ker π`. This is D0's recurring open generation structure, recorded as a scoped open target, not a closed result.## 04.11 SM-facing finite gauge decomposition
