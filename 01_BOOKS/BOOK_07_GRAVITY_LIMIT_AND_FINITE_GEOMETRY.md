...
`D0-LOCAL-PRESSURE-CAPACITY-DYNAMICS-001` upgrades this regime table to the local law
`Delta theta(x)=kappa(P_fb(x)-P_cap(x))`: balance preserves an existing rate, feedback excess
accelerates it, capacity excess brakes it, and sufficient excess reverses the next-step rate.
Unequal local balances give unequal local accelerations, so this statement does not assume a
homogeneous universe. The paired `D0-PACKING-LIMIT-HOMOGENEITY-NOGO-001` proves that a common
density ceiling alone cannot make a black-hole interior homogeneous. Only the stronger premise of
equal saturation at every bulk site would do so; boundary-capacity saturation does not provide it.

The physical local reading is now isolated in
`D0-LOCAL-RAYCHAUDHURI-REPRESENTATION-001`. Its explicit representation maps `kappa P_cap` to
`theta^2/3 + shearSq + activeMatterFocusing`, maps `kappa P_fb` to
`vorticitySq + measuredDarkArchiveDrive`, and maps the independently measured local acceleration
to the internal signed response. Conditional on those three equalities, Lean eliminates both
internal pressures and derives `acceleration=defocusing-focusing`, including the balance,
braking, acceleration and reversal regimes. The map is a `PHYSICS_DICTIONARY` assumption, not a
core identification.

`D0-NET-RESPONSE-DARK-DECOMPOSITION-NOGO-001` proves why the independent focusing readout is
mandatory. A single net field is invariant under the functional gauge
`(dark,focus)->(dark+g,focus+g)`, so no universal reconstruction can recover the dark component
from local expansion alone. Once focusing is supplied independently, `dark=net+focus` is unique.
...
**[Iter28 — cross-sector edge invariant (`D0-EDGE-INVARIANT-CROSS-SECTOR-001`, cert `vp_edge_invariant_cross_sector.py`).** The companion cross-SECTOR statement to the capacity roster above: the edge count `359 = e₂(9,11,13) = |E|` (origin owner `D0-SCENE-001`) is the same owned object consumed by five registered chains — the discrete EH action proxy (`D0-SCENE-SPECTRAL-ACTION-001`), the `α_top⁻¹ = 359φ⁻² − φ⁻⁵` numerator (`D0-EDGE-ALPHA-001`), the rank-3 cubic coefficient `λ³ − 359λ − 2574` (`D0-RANK3-CUBIC-SYMMETRIC-FUNCTIONS-001`), the Yukawa non-degeneracy face of the same cubic (`D0-YUKAWA-COMMUTANT-SPECTRUM-001`), and the S_DE window product `λ_cλ_r = 359/160` (`D0-SCENE-ACTIVE-EIGENVALUES-001`). Grade is COR in the `D0-XI5-CROSS-SECTOR-001` sense — a correlation of consumers of one invariant, no intertwiner (the ceiling is owned twice: the Bezout certificate `39590739579959` of `D0-SDE-CUBIC-SPECTRAL-DISJOINTNESS-001`, and "no trace/det coincidence is an intertwiner"). The same REQUIRED clause applies verbatim: the identities are structural, the value `359` is scene-specific (mutation `K(9,11,15)` moves every structural face to `399` together; the α face has no rival-scene analogue and is scope-exempt). The consumer list is not claimed exhaustive.]
...
**Golden metric Cauchy theorem and the remaining quantum-metric bridge.** The internally owned statements are now scoped at their actual mathematical generality. `D0-GHP-GOLDEN-CAUCHY-BOUND-001` proves the geometric majorant $\sum_k C\delta_0^k$ is summable for $\delta_0=1/(2\varphi^3)<1$. `D0-GHP-GOLDEN-CAUCHY-SEQUENCE-001` then proves that **any sequence in any pseudometric space** satisfying $d(x_k,x_{k+1})\le C\delta_0^k$ is Cauchy, and converges with the explicit geometric tail bound in a complete ambient space. These are genuine Lean theorems, but they do not construct a metric on the concrete D0 refinement stages.

The historical claim `D0-GROMOV-HAUSDORFF-DEQUARANTINE-001` is retained for traceability but honesty-repaired: `D0.Bridge.GromovHausdorff` now exposes the first missing object as a typed `MetricStageRealization` — actual refinement stages in an actual pseudometric space plus the stage-to-stage bound. It does **not** define a Rieffel compact quantum metric space, state-space Lip-norm structure, tunnel/bridge, quantum Gromov–Hausdorff distance, or propinquity. A genuine quantum-metric realization of the D0 finite stages and convergence in that external distance remain `D0-RIEFFEL-GHP-CONTINUUM-OWNER-001` / `ASSUMP-RIEFFEL-GHP`; identification of a smooth spin-manifold limit remains `D0-CONNES-RECONSTRUCTION-OWNER-001`. Thus the finite metric theorem is CORE, while the quantum-metric continuum is an explicit BRIDGE/PASSPORT, not a de-quarantined CORE result.
