...
**The golden Cauchy bound — honest scope after the quantum-metric audit (`D0-GHP-GOLDEN-CAUCHY-BOUND-001`, `D0-GHP-GOLDEN-CAUCHY-SEQUENCE-001`, `D0-GOLDEN-PSEUDOMETRIC-CAUCHY-OWNER-001`).** The internal theorem is a genuine geometric-series result: if a sequence in an arbitrary `PseudoMetricSpace` has successive distances bounded by `C·δ₀^k`, with `δ₀ = 1/(2φ³) < 1`, then it is Cauchy; in a complete ambient metric space it converges and has the explicit geometric tail bound. This result is fully internal and useful, but it is **not** by itself a theorem about Rieffel compact quantum metric spaces or Latrémolière propinquity.

The former `D0-GROMOV-HAUSDORFF-DEQUARANTINE-001` wording is therefore deprecated.  `D0.Bridge.GromovHausdorff` now explicitly exhibits an ordinary real-valued sequence as a scope witness and names `D0-LATREMOLIERE-QUANTUM-PROPINQUITY-TARGET-001` as the first missing object: genuine stage algebra/state spaces, an actual Rieffel Lip-norm, a valid tunnel/bridge-derived propinquity, and a D0-specific step estimate in that metric.  Those quantum-metric and smooth-reconstruction legs remain under the existing Rieffel/Connes assumption passports; no generic pseudometric theorem silently discharges them.  The finite-graph Connes=geodesic owner remains an independent finite metric anchor.
