...
**The GHP golden Cauchy bound — the internal-Cauchy half is now owned (`D0-GHP-GOLDEN-CAUCHY-BOUND-001`, `D0-GHP-GOLDEN-CAUCHY-SEQUENCE-001`, `D0-GROMOV-HAUSDORFF-DEQUARANTINE-001`).** The "D0 does not prove the refinement sequence is GHP-Cauchy" residual above is now discharged **on the internal side**: the refinement `G_k → G_{k+1}` contracts at the golden scale `δ₀ = 1/(2φ³)`, and since `0 ≤ δ₀ < 1`, the step-bound series `Σ_k C·δ₀^k` is summable (converges to `C/(1 − δ₀)`) — a genuine Lean theorem (`D0.Geometry.GHPGoldenCauchyBound`, via Mathlib `summable_geometric_of_lt_one`; cert `vp_ghp_golden_cauchy_bound.py`). This is completed in `D0-GHP-GOLDEN-CAUCHY-SEQUENCE-001` (`D0.Geometry.GHPGoldenCauchySequence`), where Lean proves `goldenTower_cauchySeq` (the sequence is topologically Cauchy), `goldenTower_converges` (it converges to a limit in any complete space), and `goldenTower_dist_le_limit` (the explicit geometric tail bound `C·δ₀^k / (1 − δ₀)`).
In `D0-GROMOV-HAUSDORFF-DEQUARANTINE-001` (`D0.Bridge.GromovHausdorff`), the quantum Gromov–Hausdorff propinquity framework of Latrémolière and Pearson–Bellissard is formalized: every refinement tower with golden contraction is unconditionally Cauchy, directly discharging the named internal residual of `ASSUMP-RIEFFEL-GHP`. So the refinement is Cauchy and convergent *for the internal step bound*. The finite-graph Connes = geodesic owner (`D0-CONNES-GRAPH-DISTANCE-OWNER-001`, cert `vp_connes_graph_distance_owner.py`, `c = 1 = edge/tick`) is the metric anchor. What stays external (prose status MECH-LIMIT, `D0-SMOOTH-MANIFOLD-INTERNAL-LIMIT-001`, PASSPORT-CLOSED): the convergence in the quantum Gromov–Hausdorff metric and the Connes reconstruction to a smooth Riemannian spin manifold — owned by `D0-RIEFFEL-GHP-CONTINUUM-OWNER-001` + `D0-CONNES-RECONSTRUCTION-OWNER-001`. The smooth manifold stays the conditional macro-shadow `D0-SMOOTH-MANIFOLD-PASSPORT-001`, never a primitive D0 input.
