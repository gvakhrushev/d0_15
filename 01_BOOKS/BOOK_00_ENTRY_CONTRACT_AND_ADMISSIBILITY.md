...
1. **2D Gleason closure.** A non-degenerate symplectic form requires a minimal 2D phase space. The owned uniqueness premise for the quadratic Born response is quarter-turn phase blindness (`QuarterTurnInvariant`). Generic area preservation does not force that quadratic; `BornAreaPreservationNoGo` (`D0-BORN-AREA-PRESERVATION-INSUFFICIENT-NOGO-001`) is the negative control. The finite closure itself is owned in §00.13.
