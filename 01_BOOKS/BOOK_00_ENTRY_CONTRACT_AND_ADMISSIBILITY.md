...
**[Five-primitive construction discipline].** Each of the five primitives received a real decision: G Wedderburn DERIVED + grading two-completion, H several towers, L orbit-selector + missing row operator, P1 no common sector, P2 external-passport. No internal operator/count/window is promoted to a physical observable.
**[D0-X5 extension layer].** D0-X5 is a transparent extension of present-core D0 by five typed primitive CONTRACTS (`extension_layer=D0-X5`), each a postulated hypothesis with a concrete non-vacuous model and axiom-deletion minimality (`D0-X5-EXTENSION-001`). A D0-X5 consequence is a theorem RELATIVE to its named contracts, never a present-core claim; present-core remains frozen and unchanged.
