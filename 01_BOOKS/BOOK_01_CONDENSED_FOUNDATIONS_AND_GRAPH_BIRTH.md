...
**[^b01-44]** — the weld chain (group structure, symmetry-class spectrum, MECH-LIMIT-D, m_s/m_d=20) is not yet discharged by a registered `vp_*` cert; the MECH-LIMIT-D aliasing gap is explicitly open. The `φ_E(44)=20=d13` count itself is certified by `05_CERTS/vp_v1141_abcd_omega8_v9_phi_capacity.py`. The empirical cross-check (lattice `m_s/m_d = 20.01 ± 0.55`, 0.03σ) and the Gatto–Sartori–Tonina Cabibbo bridge `sinθ_C = 1/√20` are downstream comparisons, not part of the forcing.
