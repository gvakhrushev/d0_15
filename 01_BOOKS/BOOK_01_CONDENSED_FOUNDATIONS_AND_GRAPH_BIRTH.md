...
**What remains external.**  After this closure the scene's arithmetic rests strictly on: (i) the golden toral class (owned by `φ`-rigidity, §01.14); (ii) the constant-step (centering) rule for the address ladder (owned by the orientation-parity argument of §01.22).  The three zones, the return modulus `q_T = 44`, the central capacity `m = 11`, and the collision invariant `P_2 = 371/1089` are all theorem-derived from the golden toral class and two-line verifier swap equivariance without external parameters.
