...
|---|---|---|
| boundary/mass forgetting factor `q_mass` | declared as a later quotient requirement | Books 03, 04, 08 |
...
## 01.14 Integrated theorem: П†-rigidity and dimension-graded detector quantum

This rigidity theorem belongs in the foundation book because it answers the first external objection to the П† split: whether the value can be perturbed and compensated later by a convention. Its content belongs in the foundation book because it answers the first external objection to the П† split: whether the value can be perturbed and compensated later by a convention.
...
and compare the three detector definitions which coincide at the П† point:
...
**Theorem 12.1 вЂ” П†-rigidity.**  The П† root is structurally isolated as the unique point where finite detector normalization, quadratic recursion and detector asymmetry coincide.  Perturbing the root separates these three definitions before any particle or cosmological formula is used.
...
strictly positive total response ОЈ_i ОЈ_s E_i(s)R(s) > 0
```
...
Coarse-graining is also not a new probability postulate.  If a map `ПЂ : I -> K` merges fine outcomes into a coarse outcome `k`, the coarse response is the aggregate
...
A nonlinear or power readout, for example a rule of the form `p_i в€ќ r_i^q`, is admissible only if it still satisfies the response-recovery equation
...
This is the finite D0 content of the usual `DвЂ D` response.  The probability rule is then not postulated: a finite family of amplitude channels first produces raw responses

```math
r_i=\sum_sigl(x_{i,s}^2+y_{i,s}^2igr),
```
...
A reusable shell also requires a stationary marked witness section `П‰0`. Hence
