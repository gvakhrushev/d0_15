...
**Closure.**  The linear/operator field is now explicit: the boundary residual comes from the finite boundary Euler block of the global action, not from a post-hoc scalar correction.  The old `q_mass` bridge remains a quotient dictionary only after the stationary residual has been fixed.
