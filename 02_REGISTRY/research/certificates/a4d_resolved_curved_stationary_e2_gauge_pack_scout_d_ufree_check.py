#!/usr/bin/env python3
"""F4 Track B -- gauge-canonical L≡0 packing + scout-near D, U free.
...
e2_r2_{n3,j}+e2_r3_*+D+U), specialize the three D slots to the scout-near
rationals from
`a4d_resolved_curved_stationary_e2_stationarity_specialize_scout_du_check.py`:

    D=(7/5, 5/4, 4/5)

and KEEP the six U slots free. Remaining free coordinates (11):

    (e2_r2_n3, e2_r2_j, e2_r3_n2, e2_r3_n3, e2_r3_j,
     U0, U1, U2, U3, U4, U5).

Symmetric counterpart of
`a4d_resolved_curved_stationary_e2_gauge_pack_scout_u_dfree_check.py`.

Certifies (hard wall ~8–12 min):

  * packing is the gauge-canonical one (L≡0; D scout-near; U free);
  * the eleven free-internal cleared stationarity generators, chart-denom /
    content stripped (j2^2+4, j3^2+4), have finite reduced degree on the
    open chart;
  * the three n-direction generators project under lex GB / eliminate to a
    clear open-chart obstruction, candidate, or documented stall with an
    exact next command.

Does NOT redo FIXED_13 fully-fixed D/U slices.
Does NOT redo the scout-U / D-free slice.
...
# ---------------------------------------------------------------------------
print("SECTION_GAUGE_PACK_SCOUT_D_UFREE")
...
# e2_r3_j, D0, D1, D2, U0..U5. Specialize D only; keep U free.
SPEC_D_LOCAL = {
    5: Rational(7, 5),
    6: Rational(5, 4),
    7: Rational(4, 5),
}
SPEC_D = tuple(SPEC_D_LOCAL[i] for i in range(5, 8))
REM_LOCAL = [i for i in range(14) if i not in SPEC_D_LOCAL]
# Rem: 0..4 E(2) free + 8..13 = U0..U5.
...
check("SPEC_D_COUNT_3", len(SPEC_D_LOCAL) == 3)
check("REM_COUNT_11", len(REM_LOCAL) == 11)
check("REM_ARE_E2_PLUS_U", REM_LOCAL == [0, 1, 2, 3, 4, 8, 9, 10, 11, 12, 13])
check(
    "SPEC_D_NEAR_SCOUT",
    SPEC_D == (Rational(7, 5), Rational(5, 4), Rational(4, 5)),
)
check(
    "U_STAYS_FREE",
    all(k in REM_LOCAL for k in range(8, 14)),
)
print(
    "SPEC_DECLARED: D=",
    [str(x) for x in SPEC_D],
    "; U free; remaining free =",
...
U_SYMS = {u01, u02, u03, u12, u13, u23}
CHART_FACTORS = [
    ("j2sq4", j_r2**2 + 4),
    ("j3sq4", j_r3**2 + 4),
]
...
for k, i in enumerate(FREE_IDX):
    if k in SPEC_D_LOCAL:
        chart[i] = SPEC_D_LOCAL[k]
    else:
        chart[i] = gens[gi]
        gi += 1
...
check(
    "CHART_U_SYMBOLIC",
    chart[21:] == [u01, u02, u03, u12, u13, u23],
)
check("CHART_GENS_COUNT_11", gi == 11)
...
check("INTERNAL_COUNT_11", len(internal) == 11)
...
# Prefer smallest projection. Mirror D-free: eliminate auxiliary solder
# (here U) onto (j_r2, j_r3) when possible.
if n_syms <= {j_r2, j_r3}:
...
elif n_syms <= ({j_r2, j_r3} | U_SYMS):
    # Eliminate U first: put U highest, then j's lowest.
    u_order = [v for v in (u01, u02, u03, u12, u13, u23) if v in n_syms]
    j_order = [v for v in (j_r2, j_r3) if v in n_syms]
    order_vars = u_order + j_order
    try:
        gb = sp.groebner(n_gens, *order_vars, domain="QQ", order="lex")
        gb_list = list(gb)
        print("TIMING_GB_N_WITH_U_SEC", round(time.time() - t3, 3))
        print("GB_N_DIRECTION_WITH_U", gb_list)
        print("GB_ORDER_VARS", [str(v) for v in order_vars])
    except Exception as exc:  # pragma: no cover - defensive
        print("GB_N_WITH_U_FAILED", type(exc).__name__, str(exc)[:200])
        gb_list = None
else:
    # Broader support: n's + j's + U present.
    order_vars = [v for v in gens if v in n_syms]
    # Prefer n's / U highest to eliminate onto (j, ...).
    try:
        gb = sp.groebner(n_gens, *order_vars, domain="QQ", order="lex")
        gb_list = list(gb)
...
j2_plus_j3_in = False
# With D specialized nonzero, open-chart units are trivial; U is unipotent.
open_units = set()


def forces_j3sq4_on_open_chart(expr):
    """True if expr forces j3^2+4=0 on the open chart (D fixed nonzero).

    After stripping QQ content, remaining poly is a positive power of
    (j_r3^2+4) (possibly times a QQ unit), with no other essential factors.
    """
    e = sp.expand(expr)
    if e == 0:
        return False
    ...
    if set(rem_fs) <= {j_r3} or set(rem_fs) <= {j_r2, j_r3}:
        Pr = Poly(
            rem,
            *([j_r2, j_r3] if j_r2 in rem.free_symbols else [j_r3]),
            domain="QQ",
        )
    ...
    while True:
        try:
            Pl = (
                Poly(leftover, *sorted(leftover.free_symbols, key=str), domain="QQ")
...
# If still unsettled: resultant eliminating U onto (j_r2, j_r3), then
# optional n-focused GB under remaining wall (avoid full-11 blind GB).
if outcome in ("N_GB_NO_PURE_J", "J_LOCUS_OPEN_CANDIDATE", "N_GB_STALL", "UNKNOWN"):
    abort_if("before resultant / elim")
    if n_syms & U_SYMS and ({j_r2, j_r3} & n_syms):
        t4 = time.time()
        elim = list(n_gens)
        for var in (u01, u02, u03, u12, u13, u23):
            ...
            for a, b in zip(elim, elim[1:]):
                try:
                    ...
                    print(f"RESULTANT_{var}_SKIP", type(exc).__name__, str(exc)[:120])
                    continue
                r = sp.expand(r)
                if r != 0:
                    new_elim.append(r)
        ...
        for r in elim:
            if forces_j3sq4_on_open_chart(r):
                ...
                check("OPEN_CHART_EMPTY_N_PROJECTION", True)
                break
            rf = sp.factor(sp.expand(r))
            print("RESULTANT_FACTORED", rf)
            # Also accept pure-j elements that force chart-closed.
            if set(sp.sympify(r).free_symbols) <= {j_r2, j_r3}:
                if forces_j3sq4_on_open_chart(r):
                    j3sq4_in_ideal = True
                    outcome = "CHART_CLOSED_EMPTY"
                    check("J3SQ4_VIA_RESULTANT_JONLY", True)
                    check("OPEN_CHART_EMPTY_N_PROJECTION", True)
                    break

# Optional: GB on the three n-gens only with a j-favoring order if still open
# and wall allows. Do NOT attempt full-11 free-internal GB (too heavy).
if outcome in ("N_GB_NO_PURE_J", "J_LOCUS_OPEN_CANDIDATE", "UNKNOWN") and elapsed() < WALL_SEC - 120:
    abort_if("before n3-focused GB")
    t5 = time.time()
    try:
        # Put U+n highest, j lowest.
        order_n = (
            [v for v in (n3_r2, n2_r3, n3_r3) if v in n_syms]
            + [v for v in (u01, u02, u03, u12, u13, u23) if v in n_syms]
        ...
        for v in order_n:
            if v not in seen:
                ...
                order_n2.append(v)
        for v in gens:
            if v in n_syms and v not in seen:
                order_n2.append(v)
                seen.add(v)
        gb_n = sp.groebner(n_gens, *order_n2, domain="QQ", order="lex")
        gb_n_list = list(gb_n)
        print("TIMING_GB_N_REFOCUS_SEC", round(time.time() - t5, 3))
        print("GB_N_REFOCUS", gb_n_list)
        check("GB_N_REFOCUS_COMPUTED", len(gb_n_list) >= 1)
        for g in gb_n_list:
            if forces_j3sq4_on_open_chart(g):
                j3sq4_in_ideal = True
                outcome = "CHART_CLOSED_EMPTY"
                check("J3SQ4_IN_N_REFOCUS_IDEAL", True)
                check("OPEN_CHART_EMPTY_N_PROJECTION", True)
                break
            if g.free_symbols == set() and g != 0:
                outcome = "STRUCTURAL_EMPTY"
                check("N_REFOCUS_NONZERO_CONSTANT", True)
                break
            if set(g.free_symbols) <= {j_r2, j_r3} and forces_j3sq4_on_open_chart(g):
                j3sq4_in_ideal = True
                outcome = "CHART_CLOSED_EMPTY"
                check("J3SQ4_IN_N_REFOCUS_JONLY", True)
                check("OPEN_CHART_EMPTY_N_PROJECTION", True)
                break
    except Exception as exc:
        print("GB_N_REFOCUS_FAILED_OR_HEAVY", type(exc).__name__, str(exc)[:200])
        check("GB_N_REFOCUS_STALL_DOCUMENTED", True)
        if outcome == "UNKNOWN":
            outcome = "N_REFOCUS_STALL"

print("OUTCOME", outcome)

# Always certify the packing / specialization contract regardless of GB path.
check("GAUGE_PACK_SCOUT_D_UFREE_DECLARED", True)
...
print(
    "RESULT_E2_GAUGE_PACK_SCOUT_D_UFREE: under gauge-canonical L≡0 packing "
    f"and scout-near D=({','.join(str(x) for x in SPEC_D)}) with U free, "
    f"the eleven free-internal reduced gens have degrees {red_degs}; "
    f"n-dir union symbols={sorted(str(s) for s in n_syms)}; "
    f"outcome={outcome}; j3sq4_in_ideal={j3sq4_in_ideal}."
)
print(
    "SCOPE: slice under gauge-canonical packing with D scout-near / U free; "
    "not a global E2/F4 no-go; does not redo FIXED_13 fully-fixed D/U; "
    "does not redo scout-U/D-free; transverse Lorentz eqs not required for "
    "n-projection emptiness when j3^2+4 lies in the free-internal n-ideal; "
    "QR pivots still unrecovered; no continuum Einstein claim; A4 ambient "
    "FD not restarted."
)
if outcome == "CHART_CLOSED_EMPTY":
    print(
        "NEXT: both D-free (scout-U) and U-free (scout-D) scout slices "
        "chart-close under L≡0 + current E(2) NF → recompute/persist missing "
        "Jac dump for true QR pivots OR free more of the E(2) NF slots; "
        "still avoid blind 14-var / deg-33 Groebner; filter by four-channel "
        "R=R_*(C)."
    )
elif outcome == "J_LOCUS_OPEN_CANDIDATE":
    print(
        "NEXT: reconstruct exact open-chart coordinates on the recorded j-locus "
        "by back-substituting into the remaining free-internal / U / transverse "
        "gens; still minutes-scale; avoid blind 14-var GB."
    )
else:
    print(
        "NEXT: smaller projection already attempted; memo the exact stalled "
        "command; recompute Jac dump for QR pivots; or free more E(2) NF slots; "
