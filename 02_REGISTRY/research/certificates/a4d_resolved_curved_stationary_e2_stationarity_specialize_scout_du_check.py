#!/usr/bin/env python3
"""F4 Track B -- denser scout-near rational specialization of reduced E2 stationarity.
...
the nine free solder slots to low-denominator rationals nearer the numerical
E(2) Cayley-LDU scout (`a4d_curved_stationary_cayley_scout_candidate.json`):

    D0=7/5, D1=5/4, D2=4/5
    U01=1/6, U02=5/4, U03=-1/10, U12=-3/5, U13=0, U23=-1/5

(scout floats ~ D=(1.394,1.266,0.793), U=(0.170,1.265,-0.092,-0.575,0.036,-0.190))
...
Certifies:
  ...
  * the five free-internal cleared/reduced stationarity generators (chart-denom
    content stripped as in the deg-reduce / specialize-du1 certs) have degrees
    <=5;
...
Does NOT assert that D=1/U=0 and this scout-near slice exhaust the D/U space.
"""
...
# D0, D1, D2, U0..U5. Specialize D and U to scout-near rationals; retain five E(2).
SPEC_LOCAL = {
    5: Rational(7, 5),  # D0 ~ 1.394
    6: Rational(5, 4),  # D1 ~ 1.266
    7: Rational(4, 5),  # D2 ~ 0.793
    8: Rational(1, 6),  # U01 ~ 0.170
    9: Rational(5, 4),  # U02 ~ 1.265
    10: Rational(-1, 10),  # U03 ~ -0.092
    11: Rational(-3, 5),  # U12 ~ -0.575
    12: Rational(0),  # U13 ~ 0.036
    13: Rational(-1, 5),  # U23 ~ -0.190
}
SPEC_D = (SPEC_LOCAL[5], SPEC_LOCAL[6], SPEC_LOCAL[7])
SPEC_U = tuple(SPEC_LOCAL[i] for i in range(8, 14))
...
check("SPEC_D_NEAR_SCOUT", SPEC_D == (Rational(7, 5), Rational(5, 4), Rational(4, 5)))
check(
    "SPEC_U_NEAR_SCOUT",
    SPEC_U
    == (
...
check("SPEC_DISTINCT_FROM_D1_U0", SPEC_D != (1, 1, 1) or SPEC_U != (0, 0, 0, 0, 0, 0))
print(
    "SPEC_DECLARED: D=",
...
check(
    ...
    chart[18] == Rational(7, 5)
    and chart[19] == Rational(5, 4)
    and chart[20] == Rational(4, 5),
)
check(
...
print(
    "RESULT_E2_STATIONARITY_SPECIALIZE_SCOUT_DU: under provisional FIXED_13 "
    "packing and scout-near rational specialization "
    f"D=({','.join(str(x) for x in SPEC_D)}), "
    f"U=({','.join(str(x) for x in SPEC_U)}), the five free-internal reduced "
...
print(
    "SCOPE: slice-only no-go (this scout-near D/U specialization under "
    "provisional packing); not a global E2/F4 no-go; two distinct D/U slices "
    "(D=1/U=0 and this scout-near) both force chart-closed via the same GB; "
    "specialized-dir residuals (dS/dD, dS/dU) and transverse Lorentz eqs not "
    "required for emptiness (free-internal alone already forces chart-closed); "
    "QR pivots still unrecovered; no exact curved root; no continuum Einstein "
    "claim; A4 ambient FD not restarted."
)
print(
    "NEXT: recover QR pivot map / gauge-canonical packing replacing FIXED_13; "
    "or keep D free with scout-near U (or keep selected U free with scout-near "
    "D) and eliminate the degree-reduced free-internal/transverse gens; still "
    "avoid blind 14-var / deg-33 Groebner."
)
