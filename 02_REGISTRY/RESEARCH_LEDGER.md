...
| A-STRESS — quadratic matter→tensor source | IN PROGRESS | Tests whether the owned matter/localization/stress layer can generate the quadratic source required by A-RAD, rather than merely a neutral or anomaly-proportional source. | Existing ArchiveStressCoupling is anomalySum·archiveLaplacian and vanishes for anomaly-free matter; no positive conclusion is allowed until a typed nonzero source and carrier map are found. Active task: `EXP-ASTRESS-QUADRATIC-MATTER-TENSOR-SOURCE`. |
